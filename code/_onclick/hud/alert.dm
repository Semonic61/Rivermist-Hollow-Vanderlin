 ...
 */
	...
	if(master_ref)
		thealert.icon_state = "status"
		if(thealert.master_ref != master_ref)
			thealert.master_ref = master_ref
			thealert.RegisterSignal(new_master, COMSIG_ATOM_UPDATED_ICON, TYPE_PROC_REF(/atom/movable/screen/alert, on_master_update_appearance))
...
/atom/movable/screen/alert/proc/add_atom_icon(atom/atom)
	...
	atom_appearance.pixel_w = initial(atom.pixel_w)
	atom_appearance.pixel_z = initial(atom.pixel_z)
