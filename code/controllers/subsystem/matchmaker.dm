...
/datum/controller/subsystem/relations/proc/run_rival_matchmaking()
	...
	// Collect all living human minds that have a client and want at least one rival.
...
/datum/controller/subsystem/relations/proc/get_rival_candidates(datum/mind/M, list/pool)
	...
	for(var/datum/mind/C in pool)
		...
		if(C.current?.client?.prefs)
			...
			if(!isnum(their_want) || their_want <= count_rivals(C))
				continue
...
/datum/controller/subsystem/relations/proc/seed_rival_grudge(datum/mind/A, datum/mind/B, datum/relation/R)
	...
	R.add_history(new /datum/history(G.grudge_name, G.aggressor_text, G.victim_text, aggressor, victim))
	if(R_other && R_other != R)
		R_other.add_history(new /datum/history(G.grudge_name, G.aggressor_text, G.victim_text, aggressor, victim))
	qdel(G)
...
/datum/controller/subsystem/relations/proc/pick_grudge_pool(datum/mind/A, datum/mind/B)
	...
	if(job_a && job_b)
		...
		var/list/job_pool = GLOB.grudge_pool_by_job[key_ab]
		if(!length(job_pool))
			job_pool = GLOB.grudge_pool_by_job[key_ba]
		if(length(job_pool))
			return job_pool
