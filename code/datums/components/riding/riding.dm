...
/datum/component/riding
	...
	/// Passengers managed by this component; custom harness occupants are separate.
	var/list/mob/living/riders = list()
...
/datum/component/riding/proc/handle_unbuckle(mob/living/rider)
	...
	rider.set_glide_size(DELAY_TO_GLIDE_SIZE(rider.total_multiplicative_slowdown()))
...
/datum/component/riding/proc/update_parent_layer_and_offsets(dir, animate = FALSE)
	...
	var/passindex = 0
	for(var/mob/living/buckled_mob as anything in riders)
		passindex++
		update_rider_layer_and_offsets(dir, passindex, buckled_mob, animate)
	...
	if(isliving(seat))
		...
		var/list/current_offsets = living_seat.mob_offsets[BEING_RIDDEN_SOURCE]
		if(current_offsets?["x"] != px || current_offsets?["y"] != py)
			living_seat.set_mob_offsets(BEING_RIDDEN_SOURCE, _x = px, _y = py)
	else
...
/datum/component/riding/proc/update_rider_layer_and_offsets(dir, passindex, mob/living/rider, animate = FALSE)
	...
	if(carrier.buckle_dir != BUCKLE_MATCH_DIR)
		dir = carrier.buckle_dir
	if(rider.dir != dir)
		rider.setDir(dir)
	...
	// Use RMH's source-keyed offsets so scaling, posture, and other offsets still compose.
	var/list/current_offsets = rider.mob_offsets[RIDING_SOURCE]
	if(current_offsets?["x"] != x_offset || current_offsets?["y"] != y_offset)
		rider.set_mob_offsets(RIDING_SOURCE, _x = x_offset, _y = y_offset)
	rider.layer = layer
...
/datum/component/riding/proc/restore_rider_layer_and_offsets(mob/living/buckled_mob)
	...
	buckled_mob.reset_offsets(RIDING_SOURCE)
...
/datum/component/riding/proc/restore_parent_layer_and_offsets()
	...
	if(isliving(seat))
		var/mob/living/living_seat = seat
		living_seat.reset_offsets(BEING_RIDDEN_SOURCE)
	else
...
/datum/component/riding/proc/driver_move(atom/movable/movable_parent, mob/living/user, direction, actual_move_delay)
	...
	movable_parent.set_glide_size(DELAY_TO_GLIDE_SIZE(isnull(actual_move_delay) ? vehicle_move_delay : actual_move_delay))
