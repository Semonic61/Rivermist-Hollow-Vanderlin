...
/datum/component/riding/creature
	...
	/// Animals travel faster under rider control than while wandering.
	var/riding_speed_multiplier = 0.45
...
/datum/component/riding/creature/ride_check(mob/living/rider, consequences = TRUE)
	...
	if(living_parent.stat >= UNCONSCIOUS || living_parent.body_position != STANDING_UP) // if we move while on the ground, the rider falls off
		. = FALSE
	...
	living_parent.unbuckle_mob(rider, force = TRUE)

/datum/component/riding/creature/vehicle_mob_buckle(mob/living/ridden, mob/living/rider, force = FALSE)
	if(HAS_TRAIT(rider, TRAIT_CANT_RIDE))
		return
	log_riding(ridden, rider)
	if(can_use_abilities)
		setup_abilities(rider)
	...
	if(istype(ridden, /mob/living/simple_animal/hostile))
...
/datum/component/riding/creature/driver_move(atom/movable/movable_parent, mob/living/user, direction, actual_move_delay)
	if(user.buckled != parent || !ride_check(user) || HAS_TRAIT(user, TRAIT_CANT_RIDE))
	...
	if(!keycheck(user))
		...
		return COMPONENT_DRIVER_BLOCK_MOVE
	var/mob/living/living_parent = parent
	if(istype(living_parent, /mob/living/simple_animal/hostile))
		var/mob/living/simple_animal/hostile/mount = living_parent
		vehicle_move_delay = mount.move_to_delay
	var/modified_move_delay = uses_native_speed ? living_parent.total_multiplicative_slowdown() : vehicle_move_delay
	modified_move_delay *= riding_speed_multiplier
	...
	modified_move_delay = max(world.tick_lag, modified_move_delay)
	. = ..(movable_parent, user, direction, actual_move_delay = modified_move_delay)
	var/turf/next_turf = get_step(living_parent, direction)
	if(!next_turf || !turf_check(next_turf, get_turf(living_parent)) || !step(living_parent, direction))
		return COMPONENT_DRIVER_BLOCK_MOVE
	if(QDELETED(src) || user.buckled != living_parent)
		return COMPONENT_DRIVER_BLOCK_MOVE
	if(istype(living_parent, /mob/living/simple_animal))
		var/mob/living/simple_animal/mount = living_parent
		// Upkeep adds exhaustion after the riding skill discount, never before it.
		var/base_move_delay = vehicle_move_delay
		vehicle_move_delay = modified_move_delay
		if(mount.handle_ride_upkeep(user, user.m_intent == MOVE_INTENT_RUN, src))
			return COMPONENT_DRIVER_BLOCK_MOVE
		modified_move_delay = vehicle_move_delay
...
/datum/component/riding/creature/proc/force_dismount(mob/living/rider, throw_range = 8, throw_speed = 3, gentle = FALSE)
	...
	movable_parent.unbuckle_mob(rider, force = TRUE)
