...
/datum/element/ridable/Detach(atom/movable/target)
	target.buckle_lying = initial(target.buckle_lying)
...
/datum/element/ridable/proc/check_mounting(atom/movable/target_movable, mob/living/potential_rider, force = FALSE, ride_check_flags = NONE)
	...
	if(!target_movable.AddComponent(riding_component_type, potential_rider, force, ride_check_flags))
		return COMPONENT_BLOCK_BUCKLE
...
/datum/element/ridable/proc/on_stat_change(mob/source)
	...
	if(source.stat == DEAD)
		...
		source.unbuckle_all_mobs(force = TRUE)
...
/obj/item/riding_offhand/Destroy()
	...
	if(selfdeleting && rider?.buckled == carrier)
		carrier.unbuckle_mob(rider, force = TRUE)
	rider = null
	parent = null
	return ..()
...
/obj/item/riding_offhand/on_thrown(mob/living/carbon/user, atom/target)
	...
	var/mob/living/passenger = rider // Unbuckling deletes the offhand item.
	user.unbuckle_mob(passenger, force = TRUE)
	if(HAS_TRAIT(user, TRAIT_PACIFISM) || HAS_TRAIT(user, TRAIT_NO_THROWING))
		to_chat(user, span_notice("I gently let go of [passenger]."))
		return
	return passenger
