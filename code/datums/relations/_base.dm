/datum/relation
	...
	/// Keys include name, voice colour, displayed/base job, job category, honorary,
...
/datum/relation/proc/_get_job_data(mob/living/carbon/human/H)
	...
	.["honorary"] = H.honorary
	.["honorary_suffix"] = H.honorary_suffix
...
/datum/relation/proc/refresh_snapshot()
	...
	snapshot = list(
		...
		"species" = species_name ? species_name : "Unknown",
		"gender" = H.gender,
		"age" = H.age,
	)
...
/datum/relation/proc/enrich_snapshot()
	...
	snapshot["species"] = species_name ? species_name : "Unknown"
...
/datum/relation/proc/dissolve(record_history = TRUE)
	var/datum/relation/paired_relation = counterpart
	counterpart = null
	if(paired_relation)
		paired_relation.counterpart = null
		paired_relation.holder?.relations -= paired_relation
		qdel(paired_relation)
	holder?.relations -= src
	qdel(src)

/datum/relation/Destroy()
	holder?.relations -= src
	if(counterpart)
