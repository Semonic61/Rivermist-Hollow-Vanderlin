...
/datum/tgui_relations/Destroy()
	mind = null
	return ..()

/datum/tgui_relations/ui_close(mob/user)
	qdel(src)
...
/datum/tgui_relations/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("say_gossip")
			...
			if(!isnum(rel_index) || !isnum(history_index))
				return FALSE
			if(rel_index < 1 || rel_index > length(mind?.relations))
				return FALSE
			var/mob/living/M = ui.user
			if(!isliving(M) || M.mind != mind)
				return FALSE

			var/datum/relation/R = mind.relations[rel_index]
			if(!R || !R.relation_history)
				return FALSE
			if(history_index < 1 || history_index > length(R.relation_history))
				return FALSE
			...
			var/subject_name = R.snapshot?["name"]
			if(!subject_name)
				subject_name = "someone"
			M.say("Did you hear that [subject_name] [matched_gossip.heard_text]?")
