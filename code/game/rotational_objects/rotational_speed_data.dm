...
/obj/structure/proc/try_connect(obj/structure/connector)
	var/connection_result = can_connect(connector)
	if(connection_result != TRUE)
		return connection_result
	connection_result = connector.can_connect(src)
	if(connection_result != TRUE)
		return connection_result
...
/obj/structure/proc/try_network_merge(obj/structure/connector)
	...
	if(src in connector.rotation_network.connected)
		return TRUE
	var/datum/rotation_network/old_connector_network = connector.rotation_network
	var/list/to_migrate = old_connector_network.connected.Copy()
	for(var/obj/structure/child in to_migrate)
		if(src == child)
			return FALSE
		old_connector_network.remove_connection(child)
