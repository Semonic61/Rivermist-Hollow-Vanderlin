...
/obj/item/weapon/tongs/examine(mob/user)
	...
	if(held_item)
		...
		if(hott || HAS_TRAIT(held_item, TRAIT_NEEDS_QUENCH))
...
/obj/item/weapon/tongs/update_icon_state()
	...
	if(!held_item)
		...
		icon_state = "[initial(icon_state)]i[(hott || HAS_TRAIT(held_item, TRAIT_NEEDS_QUENCH)) ? "1" : "0"]"
...
/obj/item/weapon/tongs/proc/heat_held_item(source, duration, incoming, max_heat)
	...
	if(duration)
		...
		var/heated_at = world.time
		hott = heated_at
		addtimer(CALLBACK(src, PROC_REF(make_unhot), heated_at), duration)
...
/obj/item/weapon/tongs/proc/set_held_item(obj/item/new_held_item)
	if(!QDELETED(held_item))
		UnregisterSignal(held_item, list(
			SIGNAL_ADDTRAIT(TRAIT_NEEDS_QUENCH),
			SIGNAL_REMOVETRAIT(TRAIT_NEEDS_QUENCH),
	...
	if(held_item)
		...
		RegisterSignals(held_item, list(COMSIG_PARENT_QDELETING, COMSIG_MOVABLE_MOVED), PROC_REF(unset_item_on_signal))
	update_appearance(UPDATE_ICON_STATE)
...
/obj/item/weapon/tongs/proc/place_item_to_atom(atom/A, mob/user)
	...
	var/obj/item/placed_item = held_item
	placed_item.forceMove(get_turf(A))
	placed_item.remove_quench()
	set_held_item(null)
	hott = 0
...
/obj/item/weapon/tongs/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(istype(item, /obj/item/storage/crucible) \
		|| (item.tool_flags & TOOL_USAGE_TONGS) \
