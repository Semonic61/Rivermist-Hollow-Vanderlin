...
/atom/movable/proc/buckle_mob(mob/living/M, force = FALSE, check_loc = TRUE, buckle_mob_flags= NONE)
	...
	var/datum/component/riding/previous_riding = GetComponent(/datum/component/riding)
	if(SEND_SIGNAL(src, COMSIG_MOVABLE_PREBUCKLE, M, force, buckle_mob_flags) & COMPONENT_BLOCK_BUCKLE)
		if(!previous_riding)
			qdel(GetComponent(/datum/component/riding))
		return FALSE
	...
	if(M.loc != loc && !M.Move(loc))
		if(M.buckled == src)
			unbuckle_mob(M, force = TRUE, can_fall = FALSE)
		return FALSE
...
/obj/buckle_mob(mob/living/M, force = FALSE, check_loc = TRUE, buckle_mob_flags = NONE)
	. = ..()
	if(.)
		if(resistance_flags & ON_FIRE) //Sets the mob on fire if you buckle them to a burning atom/movableect
			...
			M.IgniteMob()
		if(buckle_sound)
			playsound(src, buckle_sound, 50, TRUE)

/obj/unbuckle_mob(mob/living/buckled_mob, force = FALSE, can_fall = TRUE)
	. = ..()
	if(. && unbuckle_sound)
		playsound(src, unbuckle_sound, 50, TRUE)
...
/atom/movable/proc/unbuckle_mob(mob/living/buckled_mob, force = FALSE, can_fall = TRUE)
	...
	post_unbuckle_mob(buckled_mob)
	if(can_fall && !QDELETED(buckled_mob) && !buckled_mob.currently_z_moving && isturf(buckled_mob.loc))
		var/turf/ground = buckled_mob.loc
		ground.zFall(buckled_mob)
...
/atom/movable/proc/is_buckle_possible(mob/living/target, force = FALSE, check_loc = TRUE)
	...
	if(QDELETED(src) || QDELETED(target) || !istype(target))
		return FALSE
...
/atom/movable/proc/is_user_buckle_possible(mob/living/target, mob/user, check_loc = TRUE)
	...
	if(QDELETED(user) || QDELETED(target) || !Adjacent(user) || !Adjacent(target) || !isturf(user.loc) || user.incapacitated(IGNORE_GRAB) || target.anchored)
		return FALSE
...
/atom/movable/proc/buckle_feedback(mob/living/being_buckled, mob/buckler)
	if(being_buckled == buckler)
		being_buckled.visible_message(
			span_notice("[buckler] [buckleverb]s on [src]."),
			span_notice("I [buckleverb] on [src].")
		)
	else
		being_buckled.visible_message(
			span_warning("[buckler] [buckleverb]s [being_buckled] on [src]!"),
			span_warning("[buckler] [buckleverb]s me on [src]!")
		)
...
/atom/movable/proc/unbuckle_feedback(mob/living/unbuckled_mob, mob/unbuckler)
	if(unbuckled_mob == unbuckler)
		unbuckled_mob.visible_message(
			span_notice("[unbuckler] gets off of [src]."),
			span_notice("I get off of [src].")
		)
	else
		unbuckled_mob.visible_message(
			span_notice("[unbuckler] pulls [unbuckled_mob] from [src]."),
			span_notice("[unbuckler] pulls me from [src].")
		)
