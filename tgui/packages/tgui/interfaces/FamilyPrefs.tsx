...
export const FamilyPrefs = () => {
  ...
  const [tab, setTab] = useState<'bonds' | 'filters'>('bonds');
  ...
  return (
    <Stack vertical fill>
      ...
      {currentModeConfig && currentModeConfig.label !== 'None' && (
        <Stack.Item>
          <Section title="Designated Bonds">
            <LabeledList>
                  ...
                  {genderPreferences.map((pref) => {
                    ...
                    return (
                      <Button
                        ...
                        onClick={() =>
                          act('set_gender_choice', { choice: pref.key })
                        }
                      >
                        {pref.label}
                      </Button>
      ...
      <Stack.Item>
        <Section title="Marital History">
          <Flex align="center" justify="space-between">
            <Box>
              <Icon name={data.was_divorced ? 'heart-crack' : 'heart'} mr={1} />
              {data.was_divorced
          ...
          <Box mt={1} color="label" fontSize="0.82em">
            Keeps a former spouse entry in your character&apos;s round-start
      ...
      <Stack.Item>
        <Section title="Adoption Preference">
          <Flex align="center" justify="space-between">
            <Box>
              <Icon name={data.wants_adoption ? 'child' : 'user'} mr={1} />
              {data.wants_adoption
          ...
          <Box mt={1} color="label" fontSize="0.82em">
            Allows the matchmaking system to establish adopted bonds (e.g.,
            adopted child or parent) at round-start.
          </Box>
        </Section>
      </Stack.Item>
    </Stack>
    ...
    <Stack vertical fill>
      ...
      <Stack.Item>
        <Section title="Job Groups">
          <Box color="label" fontSize="0.82em" mb={1}>
            Toggle entries below to restrict placement to specific social
            strata.
          </Box>
          <SelectionGrid
      ...
      {!noFiltersActive && (
        <Stack.Item>
          <Button fluid icon="broom" onClick={() => act('clear_all_filters')}>
            Clear All Filters
          </Button>
        </Stack.Item>
      )}
