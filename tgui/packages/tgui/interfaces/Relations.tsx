...
export const Relations = () => {
  ...
  const visible = relations
    ...
    .sort((a, b) => {
      const catDiff =
        jobCategoryRank(a.snapshot?.job_category) -
        jobCategoryRank(b.snapshot?.job_category);
      if (catDiff !== 0) return catDiff;
      return formatDisplayName(a.snapshot, a.name).localeCompare(
        formatDisplayName(b.snapshot, b.name),
      );
    });
...
const RelationCard = ({ rel }: { rel: RelationEntry }) => {
  ...
  const { rel_index, name, snapshot, desc, grudges } = rel;
  const [open, setOpen] = useLocalState<boolean>(
    `grudge_open_${rel_index}`,
    false,
  );
  ...
  const voiceColor = snapshot?.vcolor
    ? `#${snapshot.vcolor.replace('#', '')}`
    : undefined;

  return (
    <Section
      title={
        <Box inline style={{ color: voiceColor, fontWeight: 'bold' }}>
          {displayName}
        </Box>
      }
      buttons={
